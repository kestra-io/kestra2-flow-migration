...
- **Tasks calling the Kestra API now require SDK authentication:** in v1.3 tasks that call the Kestra API internally did so unauthenticated; in v2 they require explicit credentials and fail with `401 Unauthorized` without them. Two distinct failure modes hide behind this, and they need different handling.

  **(a) `auth` is `@NotNull` on the task model → 2.0 rejects the flow on save.** No server-level or namespace credential can rescue these; the flow cannot be stored at all. Read off the shipped plugin versions for 2.0.0 (`plugin-git 4.0.0`, `plugin-ee-git 2.2.0`), where the whole distinction comes from one line of class hierarchy: OSS `AbstractKestraTask` declares `@NotNull private Auth auth` while OSS `AbstractCloningTask` declares a plain `protected Auth auth` — but **EE `AbstractCloningTask extends AbstractKestraTask`**, so every plugin-ee-git task inherits the mandatory property.

  | Type | `auth` mandatory | Why |
  |---|---|---|
  | `io.kestra.plugin.git.SyncFlow` | yes | OSS, extends `AbstractKestraTask` |
  | `io.kestra.plugin.git.TenantSync` | yes | extends `AbstractKestraTask` in **both** editions |
  | `io.kestra.plugin.git.NamespaceSync` | yes on EE | ⚠️ edition-dependent — see below |
  | `io.kestra.plugin.ee.git.SyncApps` / `SyncBlueprints` / `SyncUnitTests` / `SyncDashboards` | yes | EE `AbstractSyncTask` → `AbstractCloningTask` → `AbstractKestraTask` |
  | `io.kestra.plugin.ee.git.PushApps` / `PushBlueprints` / `PushUnitTests` / `PushDashboards` | yes | EE `AbstractPushTask` → same chain |
  | `io.kestra.plugin.ee.git.Clone` | yes | EE `AbstractCloningTask` → `AbstractKestraTask`. Makes **no** API call, yet still fails to save without `auth` — the constraint is on the model, not the behaviour |

  ⚠️ **`io.kestra.plugin.git.NamespaceSync` is shipped by both plugins under the identical FQN with different constraints**: plugin-git 4.0.0 extends `AbstractCloningTask` (optional `auth`), plugin-ee-git 2.2.0 ships its own `io.kestra.plugin.git.NamespaceSync` extending EE `AbstractCloningTask` (mandatory `auth`). So the same flow is rejected on save by an EE instance and accepted by an OSS one. A flow file cannot say which edition it targets, so the tool treats it as **save-blocking**: an EE bulk deploy failing on save is the worse outcome, and on OSS the task still needs credentials from somewhere to avoid a 401 at run time.

  **(b) `auth` is optional on the model → the flow deploys and 401s at run time** unless credentials come from namespace/tenant defaults or the server config: `io.kestra.plugin.git.SyncFlows`, `Sync`, `PushFlows`, every `io.kestra.plugin.kestra.*` task (all 39 types in `plugin-kestra 2.0.3` verified — none declares a mandatory `auth`), and `io.kestra.plugin.ai.tool.KestraFlow`.

  **Type strings that do not exist in 2.0.0** and must not be matched: `io.kestra.plugin.git.SyncDashboards` and `io.kestra.plugin.git.PushDashboards` (dashboards sync is EE-only — the real types are `io.kestra.plugin.ee.git.*`, and neither plugin declares a `@Plugin(aliases = ...)` bridging them), and `io.kestra.plugin.ai.KestraFlow` (the real class is `io.kestra.plugin.ai.tool.KestraFlow`, a **nested tool** inside an agent's `tools:` list rather than a task in `tasks:`). There is **no alias** between plugin-git and plugin-ee-git — neither repo has ever carried a `@Plugin(aliases = ...)` for these types, so both sets must be listed. What does overlap is that `io.kestra.plugin.git.NamespaceSync` and `io.kestra.plugin.git.TenantSync` are shipped by *both* plugins under the identical FQN, so a single entry covers the OSS and EE builds. ⚠️ **`io.kestra.plugin.git.SyncNamespaceFiles` is conditional**, despite being listed unqualified in the customer guide: it reads and writes namespace files through `runContext.storage()`, which the worker reaches without the API, and holds no reference to `KestraClient`. It only calls the API under `includeChildNamespaces: true`, which routes through the inherited `descendantNamespaces()` → `client.namespaces().searchNamespaces()`; the property defaults to `false`. Verified in plugin-git and plugin-ee-git `main` (2026-08-28). Purge tasks are **unaffected** (they run as SystemTasks on the Executor and bypass the SDK). Credentials can be supplied at three scopes, in order of preference: namespace/tenant defaults (EE, set in the UI), the global server config (`kestra.tasks.sdk.authentication.url` / `.api-token`), or inline on the task:
  ```yaml
  - id: sync
  ...
  ```
  In worker deployments set the webserver URL explicitly, or workers target themselves. **Not automated** — credentials cannot be invented. The severity follows the split above: a task in group (a) with no inline `auth:` is **v2-incompatible** (2.0 will not save the flow, so `--disable-v2-incompatible` quarantines it), a task in group (b) is **advisory** (the flow deploys and may already be covered by server-side or namespace/tenant credentials, which the flow file cannot show). `SyncNamespaceFiles` is advisory and only when `includeChildNamespaces` is `true` or templated. Note that `auth` on a git task is the *Kestra API* credential block (`AbstractCloningTask.auth`); Git remote credentials are the separate `username` / `password` / `privateKey` properties on `AbstractGitTask`, so keying the check off `auth:` does not mis-suppress a flow that only authenticates against Git. Note this interacts with an automated rename: `io.kestra.plugin.core.log.Fetch` → `io.kestra.plugin.kestra.logs.Fetch` moves a flow *into* the `io.kestra.plugin.kestra.*` namespace that now needs auth. See `migration-guide/v2.0.0/sdk-authentication`.
...
- the reason for each warning is prepended to the flow `description`, under a `[kestra-migrate] NEEDS MANUAL REWRITE` marker, each followed by a `docs:` line linking the relevant page of the [official 2.0.0 migration guide](https://kestra.io/docs/migration-guide/v2.0.0); an existing description is preserved below a `--- original description ---` separator;
