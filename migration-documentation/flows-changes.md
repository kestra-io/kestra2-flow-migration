...
- **Tasks calling the Kestra API now require SDK authentication:** in v1.3 tasks that call the Kestra API internally did so unauthenticated; in v2 they require explicit credentials and fail with `401 Unauthorized` without them. Affected types: the plugin-git tasks that reach `AbstractCloningTask.kestraClient()` on every run — `SyncFlows`, `SyncFlow`, `Sync`, `SyncDashboards`, `PushFlows`, `PushDashboards`, `NamespaceSync`, `TenantSync` (and `Push`, already removed in v2 for other reasons) — plus the plugin-ee-git tasks `io.kestra.plugin.ee.git.SyncApps` / `SyncBlueprints` / `SyncUnitTests` / `PushApps` / `PushBlueprints` / `PushUnitTests`, every `io.kestra.plugin.kestra.*` task and `io.kestra.plugin.ai.KestraFlow`. `io.kestra.plugin.ee.git.Clone` makes no API call and is unaffected. There is **no alias** between plugin-git and plugin-ee-git — neither repo has ever carried a `@Plugin(aliases = ...)` for these types, so both sets must be listed. What does overlap is that `io.kestra.plugin.git.NamespaceSync` and `io.kestra.plugin.git.TenantSync` are shipped by *both* plugins under the identical FQN, so a single entry covers the OSS and EE builds. ⚠️ **`io.kestra.plugin.git.SyncNamespaceFiles` is conditional**, despite being listed unqualified in the customer guide: it reads and writes namespace files through `runContext.storage()`, which the worker reaches without the API, and holds no reference to `KestraClient`. It only calls the API under `includeChildNamespaces: true`, which routes through the inherited `descendantNamespaces()` → `client.namespaces().searchNamespaces()`; the property defaults to `false`. Verified in plugin-git and plugin-ee-git `main` (2026-08-28). Purge tasks are **unaffected** (they run as SystemTasks on the Executor and bypass the SDK). Credentials can be supplied at three scopes, in order of preference: namespace/tenant defaults (EE, set in the UI), the global server config (`kestra.tasks.sdk.authentication.url` / `.api-token`), or inline on the task:
  ...
  ```
  In worker deployments set the webserver URL explicitly, or workers target themselves. **Not automated** — credentials cannot be invented, and the requirement may already be satisfied server-side or at namespace/tenant level, so the tool emits an **advisory** warning (the flow deploys; it breaks at run time) for an affected task that carries no inline `auth:` (`SyncNamespaceFiles` only when `includeChildNamespaces` is `true` or templated). Note that `auth` on a git task is the *Kestra API* credential block (`AbstractCloningTask.auth`); Git remote credentials are the separate `username` / `password` / `privateKey` properties on `AbstractGitTask`, so keying the check off `auth:` does not mis-suppress a flow that only authenticates against Git. Note this interacts with an automated rename: `io.kestra.plugin.core.log.Fetch` → `io.kestra.plugin.kestra.logs.Fetch` moves a flow *into* the `io.kestra.plugin.kestra.*` namespace that now needs auth. See `migration-guide/v2.0.0/sdk-authentication`.
...
| **advisory** | Pebble `read()` / `fileURI()` using the removed `version=` argument, tasks needing SDK authentication without an inline `auth:` block | Flow deploys; breaks at run time |
