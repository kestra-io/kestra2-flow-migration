...
- **Tasks calling the Kestra API now require SDK authentication:** in v1.3 tasks that call the Kestra API internally did so unauthenticated; in v2 they require explicit credentials and fail with `401 Unauthorized` without them. Affected types: `io.kestra.plugin.git.SyncFlows`, `io.kestra.plugin.git.NamespaceSync`, `io.kestra.plugin.git.SyncNamespaceFiles`, every `io.kestra.plugin.kestra.*` task, and `io.kestra.plugin.ai.KestraFlow`. Purge tasks are **unaffected** (they run as SystemTasks on the Executor and bypass the SDK). Credentials can be supplied at three scopes, in order of preference: namespace/tenant defaults (EE, set in the UI), the global server config (`kestra.tasks.sdk.authentication.url` / `.api-token`), or inline on the task:
  ```yaml
  - id: sync
