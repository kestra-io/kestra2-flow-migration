...
- **Flow trigger `preconditions` and `timeWindow` removed:** Replace `preconditions.flows` with a top-level `dependsOn` list on the Flow trigger. Replace `preconditions.timeWindow` with a top-level `window` block using one of: `deadline`, `from`/`to`, `every`/`offset`, `lookback`. `preconditions.id` is dropped. `preconditions.resetOnSuccess` → `window.fireOnce`. The `MultipleCondition` wrapper is gone; each upstream dependency is its own `dependsOn` entry.
- **Flow trigger default states:** Verified in code on `develop` (2026-07-16, `Flow.java`): the trigger-level `states` property defaults to **all terminated states + `PAUSED`** (`SUCCESS, WARNING, FAILED, KILLED, CANCELLED, RETRIED, SKIPPED, PAUSED`), and a `dependsOn` entry's per-entry `states` has **no default** (unset = no per-entry filter; the trigger-level `states` applies). The earlier claim from the v2.0 customer guide that the default is `[SUCCESS, WARNING]` does **not** match the shipped code — flows that want only success-ish transitions should set `states: [SUCCESS, WARNING]` explicitly on the trigger or entry.
...
- **Task defaults removed:** Remove `taskDefaults` from flow definitions. Use `pluginDefaults` at the namespace or system level instead.
- **`pluginDefaults.forced` removed:** Remove the `forced` property from flow-level `pluginDefaults` entries — it is removed in v2 and causes a **hard parse failure**. Flow-level `pluginDefaults` can no longer override what a task explicitly sets; enforce non-overridable defaults at the namespace **Plugin Defaults** (EE) or global `kestra.plugins.defaults` configuration level instead. Automated: `forced` is stripped from each `pluginDefaults` entry. See `migration-guide/v2.0.0/plugin-defaults-forced-removed`.
- **`ForEach`, `ForEachItem`, `EachSequential`, `EachParallel` removed → `Loop`:** All four flow-iteration task types are removed in v2 and replaced by `io.kestra.plugin.core.flow.Loop`. This **supersedes** the earlier `EachSequential`/`EachParallel` → `ForEach` automation — `ForEach` is itself removed in v2. The migration tool does **not** auto-transform these; the rewrite is non-trivial: expressions `{{ taskrun.value }}` → `{{ item.value }}` and `{{ taskrun.iteration }}` → `{{ item.index }}`, per-iteration `outputs:` declarations with `fetchType` (outputs are no longer auto-merged), and `AllowFailure` wrappers → the `transmitFailed` property. `ForEachItem` maps to `Loop` (inline) or `Loop` + `Subflow` (isolated per-batch). Flows using any of these types are flagged with a validation warning for manual rewrite. See the customer guide `migration-guide/v2.0.0/foreach-loop`. (Pending verification against a shipped 2.0 instance.)
- **Pebble `json` filter and function removed:** Replace the `json` Pebble filter with `toJson` and the `json()` Pebble function with `fromJson()`. The `json` Pebble *test* (`{% if x is json %}`) is unrelated and still works — only the filter and function forms change.
...
- **`checks[].condition` → `checks[].when`** — flow-level `checks` rename their `condition` property to `when`, unifying it with the `when` used on tasks and triggers. A deprecated `condition` alias still parses in 2.0, so this rewrite is future-proofing rather than required; because `when` on `checks` is not recognized by v1.3, it is gated to the v2-only path and **skipped under `--stay-v1-compatible`**. Only the top-level `checks:` list is rewritten — task-level `condition` (on `If`, `Fail`, `LoopUntil`, `Switch`) is left untouched. See `migration-guide/v2.0.0/checks-condition-renamed-when`.
...
New Pebble helper functions available inside `when` expressions: `isPublicHoliday(date, countryCode, [subdivision])`, `isDayWeekInMonth(date, dayOfWeek, position)`, `isWeekend(date)`, `isLastWorkingDay(date, [workingDays])`, `dayOfWeek(date)`, `hourOfDay(date)`, `dayOfMonth(date)`, `monthOfYear(date)`.
...
| `preconditions.resetOnSuccess` | `window.fireOnce` |
