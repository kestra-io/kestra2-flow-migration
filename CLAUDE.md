...
`detectSdkAuth()` flags tasks that call the Kestra API internally (the plugin-git tasks in `sdkAuthTypes` — `SyncFlows`, `SyncFlow`, `Sync`, `SyncDashboards`, `PushFlows`, `PushDashboards`, `NamespaceSync`, `TenantSync` — the plugin-ee-git tasks `ee.git.SyncApps` / `SyncBlueprints` / `SyncUnitTests` / `PushApps` / `PushBlueprints` / `PushUnitTests`, plus any `io.kestra.plugin.kestra.*` and `io.kestra.plugin.ai.KestraFlow`) and carry no inline `auth:` block — those calls were unauthenticated on v1.3 and 401 on v2. Advisory, not v2-incompatible: credentials may already be set at namespace/tenant or server level, which the flow file cannot show. Warning-only; gated to the v2 path. `SyncNamespaceFiles` is in `sdkAuthConditional`, not `sdkAuthTypes`: it moves files via `runContext.storage()` and only hits the API under `includeChildNamespaces: true`. `git.Push` is excluded — it already reports as a removed type; `ee.git.Clone` is excluded — it makes no API call. There is no alias between plugin-git and plugin-ee-git, so EE types are listed explicitly; `git.NamespaceSync` / `TenantSync` ship from both plugins under one FQN and need only the single entry. Note `renameTypes` moves `core.log.Fetch` → `kestra.logs.Fetch`, i.e. into the namespace that needs auth.
