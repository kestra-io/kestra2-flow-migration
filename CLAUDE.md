...
- **`--summary` is on by default; `--summary=false` opts out.** The flag also switches `newDocLinks(dedupe)` off, because the summary is the thing that carries a family's link at the end — without it, deduping would print a family's link once in the whole run and drop it from every later warning. With the flag off the output is byte-identical to the pre-summary tool (verified against `main` on the 401-flow corpus, both modes), which is the property `TestDocLinks_*` in `main_test.go` guards.
