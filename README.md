...
| `--summary` | End the run with warnings grouped by family. **On by default**; `--summary=false` restores the plain per-flow output, with every warning carrying its own docs link. |
...
`--summary=false` turns the block off. Because the summary is what repeats the
docs links per family, disabling it also restores the link under *every*
warning — the output is then byte-identical to the tool's pre-summary output,
in both migration and `--check` mode.
